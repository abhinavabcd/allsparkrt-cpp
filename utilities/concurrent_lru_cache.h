...
#ifndef UTILITIES_CONCURRENT_LRU_CACHE_H_
#define UTILITIES_CONCURRENT_LRU_CACHE_H_
...
template <class TKey, class TValue, class THash>
bool ConcurrentLRUCache<TKey, TValue, THash>::remove(ConstAccessor& ac) {
  HashMapConstAccessor& hashAccessor = ac.m_hashAccessor;
  // Acquire the lock, but don't block if it is already held
  std::unique_lock<ListMutex> lock(m_listMutex, std::try_to_lock);
  if (lock) {
  ...
  return m_map.erase(ac.m_hashAccessor);
}
...
#endif /* UTILITIES_CONCURRENT_LRU_CACHE_H_ */
